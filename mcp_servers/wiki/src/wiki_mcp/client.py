...
class AzureDevOpsWikiClient:
    ...
    async def create_page(
        ...
        except Exception as e:            return WikiOperationResult(                success=False,
    ...
    async def update_page(
        ...
        """Update an existing wiki page by recreating it.
        ...
        # For Azure DevOps Wiki, the most reliable way to "update" is to delete and recreate
        # First check if the page exists
        try:
            current_page = await self.get_page(page.metadata.path, wiki_id)
            if current_page:
                # Page exists, delete it first
                delete_result = await self.delete_page(page.metadata.path, wiki_id)
                if not delete_result.success:
                    return WikiOperationResult(
                        success=False,
                        message=f"Failed to delete existing page before update: {delete_result.message}",
            ...
            logger.warning(f"Could not check existing page (will try to create): {e}")

        # Now create the page with new content
        return await self.create_page(page, wiki_id)
