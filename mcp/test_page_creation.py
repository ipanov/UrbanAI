...
def create_wiki_page():
    ...
    response = requests.get(wiki_url, headers=get_auth_header(), timeout=10)
...
---
*Generated by UrbanAI Wiki Documentation System*
"""
    ...
    print(f"API URL: {page_url}")
    
    try:
        # Use different headers for page creation
        headers = {
        ...
        if response.status_code in [200, 201]:
            ...
            # Get the page to verify
            get_url = f"{ORG_URL}/{PROJECT}/_apis/wiki/wikis/{wiki_id}/pages?path={page_path}&api-version=7.0"
            verify_response = requests.get(get_url, headers=get_auth_header(), timeout=10)
            
            if verify_response.status_code == 200:
                page_data = verify_response.json()
                print(f"✅ Page verified - Path: {page_data.get('path')}")
                print(f"   Remote URL: {page_data.get('remoteUrl')}")
                return True
            else:
                print(f"⚠️  Could not verify page: {verify_response.status_code}")
